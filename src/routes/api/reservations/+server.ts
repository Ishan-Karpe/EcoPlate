...
export async function POST(event: RequestEvent) {
  try {
    ...
    const reservation = {
      ...
      dropId,
      userId,
      dropLocation: (drop.location as string) ?? "",
      dropLocationDetail: (drop.locationDetail as string) ?? "",
      dropWindowStart: (drop.windowStart as string) ?? "",
      dropWindowEnd: (drop.windowEnd as string) ?? "",
      dropImageUrl: (drop.imageUrl as string) ?? "",
      pickupCode: code,
      status: "reserved",
